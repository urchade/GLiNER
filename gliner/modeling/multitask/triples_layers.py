...
class NormBasedInteraction(nn.Module):
    def __init__(self, dim: int, p: int = 2, power: float = 1., clamp_norm: float | None = 10.0, 
                 use_scorer: bool = False, dropout: float = 0.3):
        """
...
class TransHInteraction(NormBasedInteraction):
    ...
    """
    def __init__(self, dim: int, p: int = 2, power: float = 1., **kwargs):
...
class TuckERInteraction(nn.Module):
    ...
    def forward(self, h, r, t):
        ...
        # Core interaction: r × W → (batch, d_e, d_e)
...
class ConvKBInteraction(nn.Module):
    ...
    """
    def __init__(self, dim: int, n_filters: int = 32, dropout: float = 0.3,
                 use_bias: bool = True):
        super().__init__()
...
class ConvEInteraction(nn.Module):
    ...
    """
    def __init__(self, dim: int, emb_dim1: int, n_filters: int = 32, 
                 kernel_size: int = 3, input_drop: float = 0.2, 
                 hidden_drop: float = 0.3, feat_drop: float = 0.2,
                 use_bias: bool = True):
        super().__init__()
...
class TriplesScoreLayer(nn.Module):
    ...
    interaction_mode : str
        The type of interaction to use. Available modes:
    ...
    dim : int
        Embedding dimension (required for most interactions)
    **kwargs : dict
        Extra parameters for specific interactions:
        ...
        - Norm-based (TransE, TransH, etc.): optional p, power, clamp_norm, use_scorer, dropout
    """
    ...
    def __init__(self, interaction_mode: str, dim: int = 768, **kwargs):
        ...
        if interaction_mode == "UM":
            ...
            self.interaction = ConvEInteraction(
                dim, emb_dim1, n_filters, kernel_size, 
                input_drop, hidden_drop, feat_drop, use_bias
            )

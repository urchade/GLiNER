...
class GlinerTrainer(Module):
    ...
    def __init__(
        ...
        grad_accum_every: int | None = None, 
        ...
        max_neg_type_ratio: int = 1, # ratio of positive/negative types, 1 mean 50%/50%, 2 mean 33%/66%, 3 mean 25%/75% ...
        ...
        if not grad_accum_every:
            print(f"grad_accum_every not set, using {self.grad_accum_every} to achive a effective batch size of 64 (64 / {batch_size}) = {self.grad_accum_every}") 
    ...
    def train(self, 
        ...
        for epoch in range(num_epochs): 
            ...
            progress_bar = tqdm(enumerate(self.train_loader), desc=f'Epoch {epoch + 1}/{num_epochs}', total=len(self.train_loader))
            ...
            for batch_idx, batch in progress_bar:  
                ...
                if self.is_main and self.should_validate and divisible_by(step, self.val_every_step):  
                    ...
                    results, f1 = self.model.evaluate(self.val_data["samples"], flat_ner=True, threshold=0.5, batch_size=12,
            ...
            self.wait()
            self.print(epochOut) 
            if self.is_main and self.checkpoint_every_epoch is not None and (self.checkpoint_every_epoch == 1 or (epoch != 0 and epoch + 1 % self.checkpoint_every_epoch == 0)):
                self.model.save_pretrained(self.checkpoint_folder / f'GliNER.epoch_{epoch}_avg_loss_{avg_epoch_loss:.3f}') 
